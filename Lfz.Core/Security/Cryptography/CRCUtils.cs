namespace Lfz.Security.Cryptography
{
    ...
    public class CRCUtils
    {
        ...
        private static ushort Algorithm(ushort crc, byte cp)
        {
        ...
        public static void AddCRC(ref byte[] bufin, int n)
        {
            ushort crc16 = 0;
            int i;
            //n个数据的CRC校验
        ...
        /// </summary>
        /// <param name="bufin">信息串</param> 
        public static void AddCRC(ref byte[] bufin)
        {
            if (bufin.Length<=2) return;
            AddCRC(ref bufin, bufin.Length-2);
        }
        ...
        public static bool CheckCRC(ref byte[] bufout, int count)
        {
            ushort crc16 = 0;
            int i;
            for (i = 0; i < (count - 2); i++)
                crc16 = Algorithm(crc16, bufout[i]);
        ...
        /// <returns>true:校验成功,false:校验失败</returns>
        public static bool CheckCRC(ref byte[] bufout)
        {
            if (bufout.Length <= 2) return true;
            return CheckCRC(ref bufout, bufout.Length-2);
        }
